...
后续候选方向：GA 与工业约束的联合优化（把工业指标并入 fitness）。已做**有界实验**并
默认关闭（`GA_SAFE_RESTRAINT_WEIGHT` opt-in，仅 `safe_loading`）：1140 件多种子下，开启后
容器数守恒、横向固定力与危险簇稳定下降、纵向固定力多数种子大幅下降——「密度/安全互斥」
主要是 GA 对安全指标失明所致。追加实验证明启发式解本就是 GA 第 0 代的恒等个体，加上
容器数罚项后 GA 能在同箱数下把固定力压到 11.67→5.86 kN（代价是换更大容器组合，约 +33%
成本）；并暴露一个真缺口——0-100 评分器不含固定力指标，GA 排序会选出固定力爆表的高分解。
详见 `docs/ga-industrial-restraint-experiment.md`。
