...
- 当前短基准（`scripts/benchmark_solver.py --iterations 3 --warmups 1 --include-frontend`）中，`heuristic_transport` 平均约 14ms，`heuristic_cog` 平均约 15ms，并行快速档 `ga_fast` 平均约 1219ms。
- 前端默认样例已加入 benchmark（`--include-frontend`），case 名为 `frontend_default_cog`。
...
- 已将支撑候选的 x/y 区间预过滤前移到 z 顶面查询阶段；前端 438 件重心样例从约 3695ms 降到约 2293ms。
- 已增加候选最大值 counters（如 `candidate_boxes_scored_max`、`candidate_boxes_checked_max`、`candidate_points_ready_max`），用于评估是否能安全设置候选硬上限。
- 已将候选尝试从 dataclass 对象改为轻量 tuple，减少候选排序前的对象分配；前端 438 件重心样例进一步降到约 2116ms。
- 已将重叠检测从 z 层列表扫描升级为 z 层 + x/y 网格索引，候选查询只扫相交网格；前端 438 件重心样例 `overlap_scan_items` 从约 864 万降到约 69 万，耗时降到约 1423ms。
- 已内联 overlap 网格查询的 bin 计算，并仅在候选跨多个网格时分配去重集合；前端 438 件重心样例进一步降到约 1336ms。
...
- 控制极点数量：
  - 删除被其他极点支配的点。
  - 限制极点最大数量，并按策略保留最有价值的前 N 个；当前 438 件样例最大 scored 约 1690、checked 约 1143，暂不直接加硬截断。
  - 不同策略使用不同极点排序。
- 加速重叠检测：
