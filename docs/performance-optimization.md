...
- 当前短基准（`scripts/benchmark_solver.py --iterations 1 --warmups 0 --include-frontend-all`）中，`heuristic_transport` 约 11ms，`heuristic_cog` 约 16ms，并行快速档 `ga_fast` 约 1268ms。
- 前端默认样例已加入 benchmark（`--include-frontend-all`），当前默认数量为 1140 件（40/300/300/500）；最新单次结果约为：运输成本优先 8.84s、空间利用优先 8.78s、稳定性优先 5.77s、重心优先 7.77s。
...
- 已将支撑硬约束热路径合并为一次 supporters 循环，并对地面放置、不可堆叠货品和缺失支撑层做早返回，减少重复面积统计和函数调用。
- 已为重心策略增加本地化 scorer，预计算当前货品质量、容器倒数和累计重心变量，保持评分公式不变但减少候选评分属性读取开销。
- 已将 overlap z 区间层改为按 bottom z 排序的列表，候选查询在后续层不可能相交时提前停止；1140 件样例空间利用类约降到 8.3s、稳定性约 5.8s、重心约 7.5s。
- 已将候选尝试扫描从 heap pop 改为同排序键的排序迭代，候选优先级不变，减少大量 `heappop` 调用；1140 件样例重心约 7.4s、稳定性约 5.5s。
...
- 显示后端返回的 `runtime_ms`。
- 已让非 GA 求解中按钮显示当前策略名称，例如“重心均衡优先求解中”。
- 已在结果面板增加性能诊断区，展示主要阶段耗时和关键候选计数，便于定位本次求解热点。
- 已在 GA 候选方案切换时保留本次求解性能信息，避免切换候选后耗时诊断消失。
- 已将性能诊断默认折叠，并限制底部结果面板最大高度，避免压缩 3D 装载视图。
- 已扩展 CSV 导出内容：在逐件装载明细前追加方案摘要、评分指标、阶段耗时和关键计数，便于离线对比不同策略。
- 已在主指标条和性能诊断摘要中增加慢求解提示：GA 场景提示切换快速档或降低精度，非 GA 场景提示先用快速试算。
- 已优化前端回放渲染：3D 场景缓存箱体位置/尺寸/颜色；结果面板和 2D 俯视图缓存重心、过滤和装载图派生数据，减少 1100+ 件回放时的重复计算。
- 已配置 Vite 手动分包，将业务代码、React、AntD、Three、@react-three 和 Zustand 拆成独立 chunk；主业务包约 52KB，构建不再出现大 chunk warning。

## 收尾基准

2026-07-09 收尾验证：

- 前端构建通过：`cd frontend && npm run build`，主业务包约 51.5KB，无大 chunk warning。
- 后端测试通过：`conda run -n packaging python -m pytest backend -q`，128 个测试通过。
- 默认 1140 件样例单次 benchmark 命令：`conda run -n packaging python scripts\benchmark_solver.py --iterations 1 --warmups 0 --include-frontend-all`。
- 前端默认策略耗时：
  - `frontend_default_transport`：约 8839ms，`find_placement` 约 8331ms。
  - `frontend_default_max_utilization`：约 8782ms，`find_placement` 约 8273ms。
  - `frontend_default_stability`：约 5766ms，`find_placement` 约 5433ms。
  - `frontend_default_cog`：约 7772ms，`find_placement` 约 7263ms。
- 短样例耗时：
  - `heuristic_transport`：约 11ms。
  - `heuristic_cog`：约 16ms。
  - `ga_fast`：约 1268ms，完成 4 代并早停。

2026-07-10 工业策略重构后单次验证（同一 1140 件样例）：

- `frontend_default_cost` / `frontend_default_space`：约 5.7s。
- `frontend_default_safe`：约 5.4s。
- `frontend_default_delivery`：约 8.8s；包含禁止倒货依赖图与卸货终检。
- `heuristic_transport`：约 7.5ms，`heuristic_cog`：约 17ms。
- `ga_fast`：约 1.23s；基因已包含货物顺序、朝向优先级和容器顺序。

## 2026-07-14 顺序配送评分与碰撞热点优化

第 4 步消除多用容器后，4 个托盘块散成约 184 个独立单元，1140 件顺序配送耗时从 7.90s
涨到 11.74s。本轮把它压回 8.66s，且**四个策略的布局签名逐字节不变**——全部是等价改写，
不改任何决策。
...
1. **配送簇质心按世代缓存**（`ScoreContext.cluster_targets` + `epoch`）。质心在一整次
   ...
   `packer` 在换件和并入新放置时各 `epoch += 1`。
2. **评分器按入口配置特化**（`LoadingEfficiency.make_scorer`）。入口面分派原先是每个候选箱
   跑一遍字符串比较链；居中分母、站点跨度同理。全部提到构造期，按入口返回特化闭包。
   顺带发现单入口时「该面深度」与「最近入口深度」是同一个值，原先各算了一遍。
3. **重叠检测去掉跨分箱去重**（`overlaps_existing`）。实测去重集几乎去不掉任何重复
   （723 万次 `set.add` 对 682 万个待扫物体），而重叠检测一发现重叠就返回，重复看到
   同一个物体不影响结果。支撑扫描保留去重：那里重复会把支撑面积算两遍。
4. **分箱键与候选箱构造的循环开销**。`overlaps_existing` 原先每个 z 层都重建两个 `range`，
   而绝大多数箱子只落在单个分箱里；`build_attempts` 每个候选箱一次函数调用。均已展开。
...
非配送策略同步受益（第 3、4 项在所有策略共用的热路径上）。四个策略的 1140 件与 48 件
布局签名、容器数、成本、体积利用率全部不变。
