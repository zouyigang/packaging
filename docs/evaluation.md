...
- `restraint_score`：堆垛簇所需固定力的归一化安全分（越大越安全）。逐容器取
  `1 − (纵向 + 横向所需固定力) / (货重kN ×(纵向g + 横向g))`，跨容器按理论上限聚合。
  分母是「簇重心正压在支撑边界时的理论最大所需固定力」，故该分衡量实际用掉多少比例的
  「必须绑扎」额度。仅当容器配了运输加速度 profile 时非满分；否则恒为 1.0。
...
适用目标：`safe_loading`（及兼容映射到它的旧名）

满分含义：无未装货物，整体低位大底面、重心居中，且堆垛簇所需固定力低。

权重（标准模式 `validation_mode=standard`）：

- `safety_worst_score`（= min(稳定性, 重心均衡)）：50%
- `stability_score`：20%
- `balance_score`：20%
- `loaded_completion`：8%
- `used_volume_utilization`：2%

权重（工业模式 `validation_mode=industrial`）：从 `safety_worst_score` 分出 15% 给
`restraint_score`，其余不变：

- `safety_worst_score`：35%
- `restraint_score`：15%
- `stability_score`：20%
- `balance_score`：20%
- `loaded_completion`：8%
...
工业变体是唯一让「纵向固定力爆表但堆得矮（稳定性分高）」的解无法霸占高分的信号——
标准模式因固定力只在配了运输加速度时才有物理意义，不引入该维度以免凭空抬分。
背景见 [industrial-strategies.md](industrial-strategies.md) 的「评分器对固定力失明」缺口。
...
- 存在堆垛簇所需固定力（`required_stack_*_restraint_kn > 0`）且 `restraint_score < 0.60`
  时提示固定力偏高、已计入安全评分。
