...
| 第 3 步 安全装载升级 | ✅ 完成：「倾覆裕量转正」已证伪（物理不可达）；**同一箱型内**密度与安全互斥，故落为产品开关 `safety_priority`（默认关=容器优先）。开启后固定力 11.67→5.53 kN、危险簇 27→17、细长比 11→8。**后补容量换安全**（`_score_safety_trials`）：安全优先在工业模式下按实测固定力择箱，用 40GP+20GP 取代 3×20GP，同样 5.53 kN 但容器 3→2、成本 6080→5480 |
...
**阶段四已收尾（第 0~6 步全部完成）**。此后又补了几项：前端 Playwright 端到端回归；多容器类型混装的开箱选择（默认样例新增 40GP，成本策略改用 1 只 40GP 取代 2 只 20GP，成本 4000→3400）；**评分器补固定力维度**（工业模式 `safe_loading` 的 0-100 评分新增 `restraint_score`，权重从 `safety_worst` 分 0.15，GA 与多候选排序不再把固定力爆表的解排到第一；只碰评分不碰装载，门禁全绿）；**容量换安全**（安全优先在工业模式下按实测固定力择箱 `_score_safety_trials`，用 40GP+20GP 取代 3×20GP，同样 5.53 kN 但容器 3→2、成本 6080→5480）。

后续候选方向（尚未排定优先级，需先确认业务价值）：GA 与工业约束的联合优化——目前 GA 只搜「放置顺序 + 朝向」，工业硬约束仍靠解码器逐候选拒绝。已做**有界实验**（把堆垛簇所需固定力并入 GA fitness，仅 `safe_loading`，环境变量 `GA_SAFE_RESTRAINT_WEIGHT` opt-in，**默认关闭**）：1140 件、pop24/gen12、3 个种子下，开启后**容器数守恒**，横向固定力与危险簇 3/3 种子全部下降、纵向固定力 2/3 种子大幅下降（中位约 −53%）——「密度/安全互斥」很大程度是 GA 对安全指标失明的产物。追加实验进一步查明：GA 初始种群的恒等个体**就是**启发式解（旧说法「GA 3 箱不及启发式 2 箱」有误——2 箱解一直在候选池，是最终排序按 0-100 评分弃选了它）；给 fitness 加容器数罚项后，GA 两种子都在**同箱数**下找到固定力显著更低的解（11.67→5.86/9.29 kN），但改善全部来自换更大容器组合（2×20GP→40GP+20GP，约 +33% 成本）；严格同组合下改善微弱且不稳定——高密度下的固定力下限是真物理。同时暴露真缺口：**0-100 评分器不含固定力指标**，GA 排序会选出纵向固定力爆表（15~17 kN）的高分解。详见 `docs/ga-industrial-restraint-experiment.md`。**该缺口已修复**（评分器补 `restraint_score`，见上段），实验建议的「容量换安全」也已落地（`_score_safety_trials`）；GA 侧仍未做的只剩「把容器数/成本意识并入 GA fitness」——但 GA 单次 11+ 分钟、业务价值待定，暂缓。
