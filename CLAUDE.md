...
| 第 3 步 安全装载升级 | 🔶 部分（已重定目标）：策略合并 + 装后居中已有。实测 `safe_loading` 的安全指标反而略差于 `cost_efficiency`（危险簇 27 vs 26），因为它的评分是纯几何键；但「让倾覆裕量转正」已证伪（物理不可达），目标重定为「不牺牲容器数地降低所需固定力」 |
...
**下一步（单一最高优先级）**：第 3 步「安全装载升级」，按重定后的目标推进。已知事实：`safe_loading` 的放置评分是纯几何键 `(z, -底面积, y, x)`，对安全指标毫无感知，实测安全性反而略差于 `cost_efficiency`（危险簇 27 vs 26、细长比 11.0 vs 10.5）——缺口是真的。但两条已证伪的路不要重走（详见 `docs/industrial-strategies.md`「安全装载：倾覆裕量为什么不能转正」）：① 让倾覆裕量转正 = 物理不可达（box-D 底面半宽 100mm、纵向 0.8g ⇒ 重心高 ≤125mm ⇒ 只能码一层）；② 在「同 z、同底面积」的候选里做同档择优 = 会多用 1 个容器，因为该档位**不是**密度中性的。可行方向：改 `order_placeables`（放置顺序）与朝向优先级这类真正密度中性的维度。完成标准：`required_stack_*_restraint_kn` 与 `risky_stack_cluster_count` 下降、容器数保持 2、装载完成率与耗时不回退。
