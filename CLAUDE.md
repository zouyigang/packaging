...
**下一步（单一最高优先级）**：第 5 步「策略对比门禁」——收紧各策略的质量阈值。当前 1140 件门禁只挡「装不完 / 有工业错误码 / 容器数 > 3 / 托盘悬空 / 布局不确定」，不挡质量回退：任何一次改动只要还能装完，哪怕体积利用率、稳定性、固定力需求全面变差也能过。目标是给每个策略的关键指标（容器数、成本、所需固定力、危险簇数、体积利用率、耗时）定下允许退化阈值并写进基准脚本，超阈值即失败。完成标准：故意注入一次退化（如把 `safe_loading` 换回纯几何序）能被门禁拦下并报出具体指标。其后做第 6 步的 `feasible/partial/infeasible` 语义分层与 AntD `destroyOnClose` 迁移。
