...
**下一步（单一最高优先级）**：第 6 步「评估与界面」——收尾阶段四。两件事：① `feasible / partial / infeasible` 的语义分层：目前 `status` 与工业错误码、风险告警混在一起，前端无法区分「算法没装完」「装完但不可执行」「装完可执行但有风险提示」，需要在 `Solution` 上把错误 / 风险 / 提示分层返回并在 `ResultPanel` 分层展示；② 清理前端弃用告警（AntD Modal 的 `destroyOnClose` → `destroyOnHidden`）。完成标准：三种状态在接口与界面上语义一致、能定位到具体设备/货品/站点，前端构建无弃用告警。
